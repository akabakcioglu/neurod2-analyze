...
while (<>){			# read one sequence at a time
    chomp;
    my ($id,$nmerge,$score,$chr,$start,$end,$lmerge,$mstart,$mend,@mergelist) = split/\t/; # for merge_50
...
for my $chr ("chr1","chr2","chr3","chr4","chr5","chr6","chr7","chr8","chr9","chr10","chr11","chr12","chr13","chr14","chr15","chr16","chr17","chr18","chr19","chrX","chrY","chrM") {
	...
	if (1) {
#	if ($hpeak{$id}{score} > 500) {# && $hpeak{$id}{end}-$hpeak{$id}{start} < 500) {
	    my $subseq = $obj->subseq($hpeak{$id}{start}=>$hpeak{$id}{end});
#	my $revseq = revcom_as_string($subseq);
	    print ">$id\n$subseq\n";
#	    print "$subseq\n";
	}
    }
