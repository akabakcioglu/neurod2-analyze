...
my $lentotal=0;			# total length of transcripts
...
my $lenperpeak = 1.0*$lentotal/$npeak;
print STDERR "total transcript length = $lentotal\n";
...
for my $reg (keys %hreglist) {
	...
	if (			#$hmean{$reg}{$ord}>1  && 
	    ...
	    my $p = 1.0*$hreglen{$reg}{$ord}/$lentotal;
	    my $mean = $npeak*$p; # expected hits
	    my $std =  sqrt($npeak*$p*(1-$p)); # standard deviation
	    my $erfarg = 1+($npeaks-$mean)/(sqrt(2)*$std);
	    my $aconst = 8*(PI-3)/(3*PI*(4-PI));
	    ...
	    print join("\t",
		       ...
		       $std,
		       ($npeaks-$mean)/$std,
		       erf($erfarg)==1?($erfarg<=>0)*$erfarg*$erfarg*($aconst*$erfarg*$erfarg+4/PI)/($aconst*$erfarg*$erfarg+1)/log(10.0) + log(2.0)/log(10.0):-($erfarg<=>0)*log(1-erf($erfarg))/log(10.0),
		       join(",",keys %hreggenes),
		       join(",",uniq @{$hreglist{$reg}{$ord}})
		)."\n" if ($hreglen{$reg}{$ord} > $lenperpeak);
	}
    }
}
